document.addEventListener('DOMContentLoaded', function() {
  ...
  let gameActive  = true;    // ← ensure this is declared so addAIMessage can check it
  ...
  function startEasyRound(){
    ...
    if(easyRounds>=3) return; // handled in answer handler
    phase='easy';
    let candidates=Object.keys(nflToCollege).filter(name=>{
      ...
      return (qbOK||skillOK)&&!playerExclusions.includes(name.toLowerCase());
    });
  ...
  // --- Handle Answer (with easy→trivia fix) ---
  ...
  // --- (The rest of your trivia/binary rounds and askNextQuestion, show/hideBinaryChoices, etc. remain unchanged) ---
  // ...  
});
